...
m_crew = 8 * 85;                                    % [kg]           (massa media di 85 kg per persona)
    ...
    % ed un range in crociera pari a 11000 [km], considerando che, diminuendo in parte
...
figure()
hold on
line("xdata", [W_S, W_S], "ydata", [-1000, 1000], "linewidth", 1.5)
xlabel('W/S [kg / m²]')
ylabel('T/W')
...
legend('STALLO', 'CRUISE', 'TOP', 'ROC', 'CEILING', 'Design Point', 'Location', 'best')
    ...
    % Si sceglie come punto di design y3 che si vede dal matching chart
    % essere il punto che permette di minimizzare la superficie dell'ala a
    % discapito di un leggero aumento di spinta necessaria. Siccome sul
    % motore non abbiamo problemi di spinta disponibile, mentre sull'ala ci
    % veniva un'apertura della madonna abbiamo scelto così
...
m_in_c = coeff(1)*coeff(2)*m_to_design;                                         % massa inizio crociera
m_fin_c = coeff(1)*coeff(2)*coeff(3)*m_to_design;                               % massa fine crociera

m_avg_tmp = (m_in_c+m_fin_c)/2;                                                 % massa media di crociera [kg]
...
CL_MAX = 2 * m_to_design * g / (rho_0 * (v_stallo * 1.1) ^2 * S);               % Si moltiplica la velocità per 1.1 dato che la formula è molto conservativa, quindi si 
                                                                                % aumenta tale velocità per portarsi a valori tipici di quelli al decollo ma comunque tenendo un margine
                                                                                % rispetto alla velocità effettiva di decollo che va moltiplicata per 1.2, ma ci teniamo conservativi essendo
...
CL_MAX_gross = CL_MAX_W / k_a;
    ...
    % 1.3 il valore degli HLD, per cui si entra con un
    % CL=CLMAXgross-CL_HLD(1.3)=1.40
    % (0.4 slat 0.9 flap) Sadrey pg 236

deltaCLHD = 1.4;                                                                % prendiamo 1 di Fowler Flap e 0.3 di Leading Edge slat dal Sadrey ( prendiamo i valori più bassi per essere conservativi)
Cl_max = CL_MAX_gross - deltaCLHD;
visc_din = 1.46e-5;
Reynolds = rho * v * 8 / visc_din;

    % Tra i profili segnalati viene inizialmente scelto il GOE 682, con uno sweep angle di
    % 30 deg, come suggerito dal Raymer (pag. 53). Dopo averlo scelto ci si
    % è resi conto che il calettamento sarebbe stato di circa un grado (
    % valori classici compresi tra 3 e 5 gradi) quindi si è scelto di
    % cambiare profilo e muoversi verso il NACA1412. Esso garantisce il
    % Cl_id a 3.5 deg e ha un Cl_max di 1.51 a fronte di un 1.3 richiesto.
    % Ciò permette di avere un margine di lontananza dallo stallo
    % abbastanza safe. Inoltre, i valori presi da AirfoilTools sono a
    % Reynolds 1'000'000 (valore massimo disponibile) e considerando la
    % corda media aerodinamica che successivamente viene calcolata che
    % risulta di circa di 8 m si vede come otteniamo un Re di 44 milioni quindi i dati
    % presi dal sito possono essere utilizzati
    

%% Dimensionamento geometrico ala
b = sqrt(AR * S);                                                                % [m]
S_w_ang = 29;                                                                    % [deg]
S_w_ang_te = 18;                                                                 % [deg]
Gamma = 5;                                                                       % [deg]
...
%AGGIUNGERE SPESSORE DI 102MM AD OGNI LATO (COME DA NORMATIVA) per lo
%spessore



    % Lunghezza del Nose (fare riferimento a pag. 429 del PDF del Saedray)
...
y1_tail = c_root_tail - b_tail/2 * tand(Sweep_tail);
y2_tail = y1_tail - c_tip_tail;
figure()
plot([0 b_tail/2], [0 y2_tail])
hold on
plot([0 b_tail/2], [c_root_tail y1_tail])
plot([0 0], [0 c_root_tail])
plot([b_tail/2 b_tail/2], [y2_tail y1_tail])
grid on
xlabel('asse y')
ylabel('asse x')
...
%% Dimensionamento motore
    ...
    %%  Feasibility: the operational perspective
    
%     %%% si calcola la distanza di Ground Roll
%     
%     mu_gr = 0.05;    % Coeff di attrito volvente -> da tabella a slide 19 della prof -> dry concrete/asphalt
%   
%     %Dalle CS25 si prende come valore di v_2=1.2*velocità di stallo
%     
%     v_2 = 1.2*v_stallo;
%     
%     % come valore di Thrust si prende il valore massimo che riescono a
%     % erogare i motori, ovvero il valore necessario al decollo
%     
%     T1 =T;
%     C_D_0 = 0.015;
%     a_gr = g*(T1/(g*m_to_design)-mu_gr);
%     b_gr = (C_D_0 - (mu_gr^2)/(4*K))*(rho_0*g)/(2*m_to_design*g/S);
%     
%     d_R = (1/(2*b_gr))*(log(a_gr)-log(a_gr-b_gr*(v_2^2)));
% 
%     %%% si calcola la distanza di Take OFf Distance
%     % Da pagina 705 del PDF del Raymer si è ricavato un tempo di rotazione
%     % pari a 3 secondi
%     
%     t_LO = 3;
%     d_LO=t_LO*v_2;
%     n_z=0.8*v_2^2/(v_stallo)^2; 
%     
%     R_curv=v_2^2/(g*(n_z-1))
%     h0=35*0.3048;
%     gamma_TO=acosd(1-h0/R_curv);
%     d_m =R_curv*sind(gamma_TO)
%     d_TO=d_R+d_LO+d_m 
%     
    %Il risultato ottenuto è stato raggiunto grazie AL DIO GILI
    %(Esercitazione 5 di FMV (matteo luci sfigato che non l'ha fatta) --->
    %VEDERE QUELLA
%     v_fail=linspace(20,110);
%     a_AEO=a_gr;
%     a_OEI=g*(0.5*T/(m_to_design*g)-mu_gr);
%     d_OEI= 1/(2*b_gr)*(log(a_OEI-b*v_fail.^2)-log(a_OEI-b_gr*v_2^2));
%     d_AEO= 1/(2*b_gr)*(log(a_AEO)-log(a_AEO-b_gr*(v_fail.^2)));
%     d_AeG= d_AEO+d_OEI+d_LO+d_m;
%     t_react=2;            %da Gili
%     n_x=0.3;              %Da Gili (attesa risposta mail)
%     d_react= v_fail.*t_react;
%     d_break= v_fail.^2/(2*g*n_x);
%     d_AeS=d_AEO+d_react+d_break;
%     
%     figure()
%     hold on
%     grid on
...
W=m_to_design*g; %peso in N
CL_TO=2; %Coefficiente di portanza in take off
%T_12= 379000; %N per motore Trent XWB 84
T_12= 402920; % N con motore TRENT 1000-R3 (2017)
M=2; %Numero motori
T=T_12*M; %Total thrust
f=0.05; %Coefficiente di attrito
rho=1.225; %densità
h0=10.67; %distanza ostacolo in metri
k=1/(pi*e*AR);
Cl=f/(2*k);
...
A_gr= g*(T/W - f);
B=(rho*g)/(2*W/S)*(C_D_0 - (f^2)/(4*k));
d_rull = 1/(2*B)*(log(A_gr)-log(A_gr-B*(V_R^2)))
d_liftoff=2*V_R
n_z=(0.734*V_R^2)/(V_min^2); %perche si ha una V2 maggiore 0.734 in modo da avere nz=1.15
R=(V_R^2)/(g*(n_z-1));
cosgamma=1-(h0)/R;
gamma=acos(cosgamma);
d_H=R*sin(gamma);
D_takeoff=d_rull+d_liftoff+d_H

%% BFL
V_fail=linspace(0,V_R,1000);
d_aeo=1/(2*B)*(log(A_gr)-log(A_gr-B*(V_fail.^2)));
%Nel tratto successivo cambia A
A_oei= g*(0.5* T/W-f);
d_oei= 1/(2*B)*(log(A_oei-B*(V_fail.^2)) - log(A_oei-B*(V_R^2)));
d_AAG= d_aeo+d_oei+d_H+d_liftoff;
d_reaz=2*V_fail;
d_stop=(V_fail.^2)/(2*g*n_x);
d_AAS=d_aeo+d_reaz+d_stop;
figure()
...
% L'ultimo passo da fare è ricavare la Take Off Run, che da CS-25.113C
% Emendamento 2 è definita come la distanze percorsa da inizio
% decollo e il punto che si trova nel mezzo del punto in cui si è raggiunta
% la velocità di lift off e il punto in cui l'aereo ha raggiunto la quota
% di 35 feet
d_TO_run = (d_rull+d_liftoff+d_H/2);


% si plotta il grafico della Take-Off Distance e della Goll
xc = d_rull + d_liftoff;
yc = R;
xx = linspace(xc,D_takeoff);
yy = yc - sqrt(R^2-(xx - xc).^2);
xx1 = linspace(0,xc);
...
ylabel('Quota [m]')
title('Take Off Distance')
legend('Traiettoria effettiva del velivolo','Take off Distance','Take Off Run','Location','best')
